...
public abstract class AbstractApplicationContextTests extends AbstractListableBeanFactoryTests {
	...
	public void testMessageSource() throws NoSuchMessageException {
		assertEquals(applicationContext.getMessage("code1", Locale.getDefault()), "message1");
		assertEquals(applicationContext.getMessage("code2", Locale.getDefault()), "message2");

		try {
			applicationContext.getMessage("code0", Locale.getDefault());
